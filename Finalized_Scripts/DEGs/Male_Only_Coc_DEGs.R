...
Males_Only <- subset(NAc_Combo,idents = "Male")

#MAke the identities of the males only object the celltype
Idents(Males_Only) <- Males_Only$Combo_CellType

DimPlot(object = Males_Only,reduction = "umap",label = TRUE) + NoLegend()
...
Males_Only$sample.id <- as.factor(paste(Males_Only$Dataset,Males_Only$Sex_Stim,sep = "_"))

#Get cell and sample metrics for aggregation
Males_Only@meta.data$CellType <- as.factor(Males_Only@meta.data$Combo_CellType)
cell_names <- purrr::set_names(levels(Males_Only@meta.data$CellType))
cell_names
...
Males_Only$sample.id <- factor(Males_Only$sample.id)
sample_ids <- purrr::set_names(levels(Males_Only$sample.id))
sample_ids
# Acute_Male_Coc      Acute_Male_Sal   Repeated_Male_Coc   Repeated_Male_Sal 
# "Acute_Male_Coc"    "Acute_Male_Sal" "Repeated_Male_Coc" "Repeated_Male_Sal" 
...
table(Males_Only@meta.data$sample.id)
# Acute_Male_Coc    Acute_Male_Sal Repeated_Male_Coc Repeated_Male_Sal 
# 3184              3905              5737              5589 
...
mapply(FUN = function(x, z) {
  ...
  file_name_pca <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Male_Coc/PCA_", z)
  ...
  file_name_heatmap <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Male_Coc/sample_to_sample_heatmap_", z)
...
dds_all_cells <- mapply(FUN = function(x, z) {
  ...
  file_name_dist <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Male_Coc/DispPlot_", z, ".pdf")
...
Idents(Males_Only) <- gsub(Idents(Males_Only),pattern = "-",replacement = ".")
SeuratCounts <- t(GetAssayData(object = Males_Only,slot = "data",assay = "RNA"))
for(i in names(res_all_cells)){
  ...
  Genes_PctExp <- as.data.frame(colMeans(SeuratCounts[WhichCells(object = Males_Only,idents = i),res_all_cells[[i]]$Gene]>0)*100)
...
mapply(FUN = function(x, z) {
  ...
  file_name <- paste0("2019-JD-0040/MCN_Code/Tables/DESeq2_Male_Coc/DESeq2_res_", z, ".csv")
...
mapply(FUN = function(x, z) {
  ...
  file_name_norm <- paste0("2019-JD-0040/MCN_Code/Tables/Normalized_Counts/DESeq2_Male_Coc/normalized_counts_", z, ".csv")
...
write.table(x         = DEGs_DF,
            file      = "2019-JD-0040/MCN_Code/Tables/DESeq2_Male_Coc/No_DEGs_Per_CellType.txt",
            sep       = "\t",
            col.names = TRUE,
            row.names = FALSE,
            quote     = FALSE)
