...
Idents(NAc_Combo) <- NAc_Combo$Sex
Females_Only <- subset(NAc_Combo,idents = "Female")

#MAke the identities of the Females only object the celltype
Idents(Females_Only) <- Females_Only$Combo_CellType

DimPlot(object = Females_Only,reduction = "umap",label = TRUE) + NoLegend()
...
Females_Only$sample.id <- as.factor(paste(Females_Only$Dataset,Females_Only$Sex_Stim,sep = "_"))

#Get cell and sample metrics for aggregation
Females_Only@meta.data$CellType <- as.factor(Females_Only@meta.data$Combo_CellType)
cell_names <- purrr::set_names(levels(Females_Only@meta.data$CellType))
cell_names
...
Females_Only$sample.id <- factor(Females_Only$sample.id)
sample_ids <- purrr::set_names(levels(Females_Only$sample.id))
sample_ids
# Acute_Fem_Coc      Acute_Fem_Sal   Repeated_Fem_Coc   Repeated_Fem_Sal 
# "Acute_Fem_Coc"    "Acute_Fem_Sal" "Repeated_Fem_Coc" "Repeated_Fem_Sal" 
...
table(Females_Only@meta.data$sample.id)
# Acute_Fem_Coc    Acute_Fem_Sal Repeated_Fem_Coc Repeated_Fem_Sal 
# 4895             3671             6537             5736 
...
mapply(FUN = function(x, z) {
  ...
  file_name_pca <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Female_Coc/PCA_", z)
  ...
  file_name_heatmap <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Female_Coc/sample_to_sample_heatmap_", z)
...
dds_all_cells <- mapply(FUN = function(x, z) {
  ...
  file_name_dist <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Female_Coc/DispPlot_", z, ".pdf")
...
Idents(Females_Only) <- gsub(Idents(Females_Only),pattern = "-",replacement = ".")
SeuratCounts <- t(GetAssayData(object = Females_Only,slot = "data",assay = "RNA"))
for(i in names(res_all_cells)){
  ...
  Genes_PctExp <- as.data.frame(colMeans(SeuratCounts[WhichCells(object = Females_Only,idents = i),res_all_cells[[i]]$Gene]>0)*100)
...
mapply(FUN = function(x, z) {
  ...
  file_name <- paste0("2019-JD-0040/MCN_Code/Tables/DESeq2_Female_Coc/DESeq2_res_", z, ".csv")
...
mapply(FUN = function(x, z) {
  ...
  file_name_norm <- paste0("2019-JD-0040/MCN_Code/Tables/Normalized_Counts/DESeq2_Female_Coc/normalized_counts_", z, ".csv")
...
write.table(x         = DEGs_DF,
            file      = "2019-JD-0040/MCN_Code/Tables/DESeq2_Female_Coc/No_DEGs_Per_CellType.txt",
            sep       = "\t",
            col.names = TRUE,
            row.names = FALSE,
            quote     = FALSE)
