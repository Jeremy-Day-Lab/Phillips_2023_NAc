...
cell_names
# Polydendrocyte              Olig-1               Mural           Microglia           Astrocyte       Glutamatergic     Sst-Interneuron   Pvalb-Interneuron 
# "Polydendrocyte"            "Olig-1"             "Mural"         "Microglia"         "Astrocyte"     "Glutamatergic"   "Sst-Interneuron" "Pvalb-Interneuron" 
# Chat-Interneuron           GABAergic            Grm8-MSN            Drd3-MSN          Drd2-MSN-2          Drd2-MSN-1          Drd1-MSN-2          Drd1-MSN-1 
# "Chat-Interneuron"         "GABAergic"          "Grm8-MSN"          "Drd3-MSN"        "Drd2-MSN-2"        "Drd2-MSN-1"        "Drd1-MSN-2"        "Drd1-MSN-1" 
...
metadata$Sex_CellType <- as.factor(paste(as.character(lapply(strsplit(as.character(metadata$sample_id),
                                                                      "_"),
                                                             "[",
                                                             1)),
                                         as.character(lapply(strsplit(as.character(metadata$sample_id),
                                                                      "_"),
                                                             "[",
                                                             3)),
                                         sep = "_"))
...
dds_all_cells <- mapply(FUN = function(x,y,z) {
  ...
  # set reference to Male
  dds$Sex <- relevel(dds$Sex, ref = "Fem")
...
mapply(FUN = function(x, z) {
  ...
  # PCA
  #-----------------Stim---------------------
  data <- plotPCA(vsd, intgroup = c("Sex_CellType"), returnData = TRUE)
  percentVar <- round(100 * attr(data, "percentVar"))
  
  pca.plot.stim <- ggplot(data, aes(PC1, PC2, color = Sex_CellType)) +
    geom_point(size = 7) +
  ...
  file_name_pca <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Sex/Stim_as_covariate/PCA_Plots/PCA_", z)
  ...
  file_name_heatmap <- paste0("2019-JD-0040/MCN_Code/Plots/DESeq2_Sex/Stim_as_covariate/Heatmaps//sample_to_sample_heatmap_",z,"")
...
ensembl <- useMart("ensembl",
                   dataset = "rnorvegicus_gene_ensembl") #Should pull the latest ensembl (This analysis was completed on 11/30/22)
...
#Not losing any DEGs when adding chromosome information. 
# [1] "Number of DEGs: 9"
# [1] "Number of DEGs: 9"                                                                                                                                                                                                        
# [1] "Number of DEGs: 2"
# [1] "Number of DEGs: 2"
...
#   [1] biomaRt_2.44.4              pheatmap_1.0.12             RColorBrewer_1.1-2          DESeq2_1.28.1               SummarizedExperiment_1.18.2 DelayedArray_0.14.1         matrixStats_0.57.0          Biobase_2.48.0             
# [9] GenomicRanges_1.40.0        GenomeInfoDb_1.24.2         IRanges_2.22.2              S4Vectors_0.26.1            BiocGenerics_0.34.0         Matrix.utils_0.9.8          Matrix_1.3-4                ComplexUpset_1.3.3         
# [17] stringr_1.4.0               dplyr_1.0.2                 Libra_1.0.0                 ggplot2_3.3.2               SeuratObject_4.0.2          Seurat_4.0.4               
# 
# loaded via a namespace (and not attached):
#   [1] blme_1.0-5             BiocFileCache_1.12.1   plyr_1.8.6             igraph_1.2.6           lazyeval_0.2.2         TMB_1.8.1              splines_4.0.2          BiocParallel_1.22.0    listenv_0.8.0         
# [10] scattermore_0.7        digest_0.6.26          htmltools_0.5.2        lmerTest_3.1-3         magrittr_1.5           memoise_1.1.0          tensor_1.5             cluster_2.1.0          ROCR_1.0-11           
# [19] limma_3.44.3           globals_0.13.1         annotate_1.66.0        tester_0.1.7           askpass_1.1            spatstat.sparse_2.0-0  prettyunits_1.1.1      colorspace_1.4-1       blob_1.2.1            
# [28] rappdirs_0.3.1         ggrepel_0.8.2          crayon_1.3.4           RCurl_1.98-1.2         jsonlite_1.7.1         genefilter_1.70.0      lme4_1.1-26            spatstat.data_2.1-0    survival_3.2-7        
# [37] zoo_1.8-8              glue_1.6.2             polyclip_1.10-0        gtable_0.3.0           zlibbioc_1.34.0        XVector_0.28.0         leiden_0.3.3           future.apply_1.6.0     abind_1.4-5           
# [46] scales_1.1.1           DBI_1.1.0              edgeR_3.30.3           miniUI_0.1.1.1         Rcpp_1.0.7             progress_1.2.2         viridisLite_0.3.0      xtable_1.8-4           reticulate_1.16       
# [55] spatstat.core_2.3-0    bit_4.0.4              htmlwidgets_1.5.2      httr_1.4.2             ellipsis_0.3.2         ica_1.0-2              farver_2.0.3           pkgconfig_2.0.3        XML_3.99-0.5          
# [64] dbplyr_1.4.4           uwot_0.1.10            deldir_1.0-6           locfit_1.5-9.4         labeling_0.3           tidyselect_1.1.0       rlang_1.0.2            reshape2_1.4.4         later_1.1.0.1         
# [73] AnnotationDbi_1.50.3   pbmcapply_1.5.0        munsell_0.5.0          tools_4.0.2            cli_3.3.0              generics_0.0.2         RSQLite_2.2.1          ggridges_0.5.2         fastmap_1.1.0         
# [82] yaml_2.2.1             goftest_1.2-2          bit64_4.0.5            fitdistrplus_1.1-1     purrr_0.3.4            RANN_2.6.1             pbapply_1.4-3          future_1.19.1          nlme_3.1-149          
# [91] mime_0.9               grr_0.9.5              xml2_1.3.2             compiler_4.0.2         rstudioapi_0.11        curl_4.3               plotly_4.9.2.1         png_0.1-7              spatstat.utils_2.2-0  
# [100] tibble_3.0.4           statmod_1.4.35         geneplotter_1.66.0     stringi_1.7.6          forcats_0.5.0          lattice_0.20-41        nloptr_1.2.2.2         vctrs_0.4.1            pillar_1.4.6          
# [109] lifecycle_0.2.0        spatstat.geom_2.4-0    lmtest_0.9-38          RcppAnnoy_0.0.19       data.table_1.13.0      cowplot_1.1.0          bitops_1.0-6           irlba_2.3.3            httpuv_1.5.4          
# [118] patchwork_1.0.1        R6_2.4.1               promises_1.1.1         KernSmooth_2.23-17     gridExtra_2.3          codetools_0.2-16       assertthat_0.2.1       boot_1.3-25            MASS_7.3-53           
# [127] openssl_1.4.3          withr_2.3.0            sctransform_0.3.2      GenomeInfoDbData_1.2.3 hms_0.5.3              mgcv_1.8-33            grid_4.0.2             rpart_4.1-15           glmmTMB_1.1.3         
# [136] tidyr_1.1.2            minqa_1.2.4            Rtsne_0.15             numDeriv_2016.8-1.1    shiny_1.5.0           
